...
export default function RH() {
  ...
  const [alerts, setAlerts] = useState([
    { id: 1, type: "cdd", text: "⚠️ Le contrat CDD de M. Ibrahima Camara expire dans 14 jours !", date: "Aujourd'hui" },
  ...
  const [newStaffForm, setNewStaffForm] = useState({
    nom: "", poste: "", salaire: "", contrat: "CDI", diplome: "", telephone: "", email: "", emploiDuTemps: "", avatar: "👨‍🏫", categorie: "Enseignant"
  });
  ...
  const handleAddEmployee = () => {
    if (!newStaffForm.nom || !newStaffForm.poste) return;
    const scheduleArray = newStaffForm.emploiDuTemps 
      ? newStaffForm.emploiDuTemps.split(";").map(s => s.trim())
      : ["Lundi au Vendredi (08h - 16h)"];
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", color: "#0f172a", padding: "4px 0 32px 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
            {alerts.slice(0, 3).map(alert => (
              <motion.div 
                ...
                style={{ 
                  background: "#fff", borderLeft: "4px solid #3b82f6", borderRadius: "10px", 
                  padding: "12px 18px", boxShadow: "0 2px 8px rgba(0,0,0,0.03)", 
                  display: "flex", justifyContent: "space-between", alignItems: "center" 
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#1e3a8a" }}>[Alerte RH]</span>
                  <span style={{ fontSize: 13, color: "#334155", fontWeight: 500 }}>{alert.text}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <span style={{ fontSize: 11, color: "#94a3b8" }}>{alert.date}</span>
                  <button 
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 800, margin: 0, background: "linear-gradient(90deg, #1e3a8a, #3b82f6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          {activeTab === "recrutement" ? (
            <button onClick={() => setShowAddCandidateModal(true)} style={{ display: "flex", alignItems: "center", gap: 8, background: "#8b5cf6", color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "0.2s", boxShadow: "0 4px 12px rgba(139,92,246,0.2)" }}>
              📥 Nouveau Candidat
            </button>
          ) : activeTab === "conges" ? (
            <button onClick={() => setShowAddLeaveModal(true)} style={{ display: "flex", alignItems: "center", gap: 8, background: "#ea580c", color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "0.2s", boxShadow: "0 4px 12px rgba(234,88,12,0.2)" }}>
              🌴 Demander un congé
            </button>
          ) : (
            <button onClick={() => setShowAddStaffModal(true)} style={{ display: "flex", alignItems: "center", gap: 8, background: "#1e3a8a", color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "0.2s", boxShadow: "0 4px 12px rgba(30,58,138,0.2)" }}>
              👤 Ajouter un Employé
            </button>
          )}
        </div>
      ...
      <div className="stats-grid" style={{ marginBottom: 28 }}>
        {[
          { label: "Membres de l'équipe", value: employes.length, sub: "Actifs, congés, etc.", icon: "👥", bg: "#eff6ff", color: "#3b82f6" },
          { label: "Enseignants Actifs", value: activeTeachersCount, sub: "Sur tous les cycles", icon: "👨‍🏫", bg: "#ecfdf5", color: "#10b981" },
          { label: "Absences du mois", value: totalAbsencesMonth, sub: "Retards et absences loggués", icon: "🚨", bg: "#fef2f2", color: "#ef4444" },
          { label: "Congés en cours", value: activeLeavesCount, sub: "Absences autorisées", icon: "🌴", bg: "#faf5ff", color: "#8b5cf6" },
        ].map((card, idx) => (
      ...
      <div style={{ 
        ...
        {[
          { id: "effectifs", label: "📋 Liste des Effectifs", color: "#1e3a8a" },
          { id: "fiches", label: "🗂️ Fiches & Évaluations", color: "#3b82f6" },
          { id: "presences", label: "📅 Suivi Présences", color: "#0d9488" },
          { id: "conges", label: "🌴 Congés & Absences", color: "#ea580c" },
          { id: "salaires", label: "💵 Paie & Salaires", color: "#16a34a" },
          { id: "recrutement", label: "💼 Recrutement & CVs", color: "#8b5cf6" },
        ].map(tab => {
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
            <div style={{ position: "relative" }}>
              ...
              <input type="text" placeholder="Rechercher par nom ou poste..." value={search} onChange={e => setSearch(e.target.value)}
                style={{ width: "100%", padding: "12px 14px 12px 42px", border: "1px solid #e2e8f0", borderRadius: 12, fontSize: 13, outline: "none", boxSizing: "border-box", background: "#fff", transition: "all 0.2s" }} />
            </div>
            
            <div>
              <select value={filterCategorie} onChange={e => setFilterCategorie(e.target.value)} style={{ width: "100%", padding: "11px 12px", border: "1px solid #e2e8f0", borderRadius: 12, fontSize: 13, outline: "none", background: "#fff", color: "#475569", fontWeight: 600 }}>
            ...
            <div>
              <select value={filterStatut} onChange={e => setFilterStatut(e.target.value)} style={{ width: "100%", padding: "11px 12px", border: "1px solid #e2e8f0", borderRadius: 12, fontSize: 13, outline: "none", background: "#fff", color: "#475569", fontWeight: 600 }}>
            ...
            <div>
              <select value={filterAnciennete} onChange={e => setFilterAnciennete(e.target.value)} style={{ width: "100%", padding: "11px 12px", border: "1px solid #e2e8f0", borderRadius: 12, fontSize: 13, outline: "none", background: "#fff", color: "#475569", fontWeight: 600 }}>
              ...
              <thead>
                <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
                  {["Nom de l'Employé", "Département / Poste", "Type de Contrat", "Masse Salariale", "Date d'embauche", "Statut", "Actions"].map(h => (
                    <th key={h} style={{ padding: "16px 20px", textAlign: "left", fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.5px" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  {filteredEmployes.map((e, idx) => (
                    <motion.tr key={e.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ borderBottom: "1px solid #f1f5f9", background: idx % 2 === 0 ? "#fff" : "#fafafa", transition: "background 0.2s" }}>
                      <td style={{ padding: "16px 20px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{e.avatar}</div>
                          <div>
                            <span style={{ fontSize: 14, fontWeight: 700, color: "#1e293b", display: "block" }}>{e.nom}</span>
                            <span style={{ fontSize: 11, color: "#64748b", display: "block", marginTop: 2 }}>{e.email}</span>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: "16px 20px" }}>
                        <span style={{ fontSize: 13, color: "#334155", fontWeight: 600, display: "block" }}>{e.poste}</span>
                        <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "#f1f5f9", color: "#475569", fontWeight: 700, display: "inline-block", marginTop: 4 }}>
                          💼 {e.categorie}
                        </span>
                      </td>
                      <td style={{ padding: "16px 20px" }}>
                        <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 20, fontWeight: 600, ...contratStyle[e.contrat] }}>{e.contrat}</span>
                      </td>
                      <td style={{ padding: "16px 20px", fontSize: 14, fontWeight: 800, color: "#1e3a8a" }}>{e.salaire.toLocaleString()} GNF</td>
                      <td style={{ padding: "16px 20px", fontSize: 13, color: "#475569" }}>{e.dateEmbauche}</td>
                      <td style={{ padding: "16px 20px" }}>
                        <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 20, fontWeight: 600, ...statusStyle[e.status] }}>{e.status}</span>
                      </td>
                      <td style={{ padding: "16px 20px" }}>
                        <button onClick={() => { setSelectedStaff(e); setDossierTab("profil"); }} style={{ background: "#eff6ff", color: "#2563eb", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "0.2s" }}>
                          👁️ Dossier complet
                        </button>
                      </td>
                    </motion.tr>
                ...
                {filteredEmployes.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ textAlign: "center", padding: "40px", color: "#64748b", fontSize: 14 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          ...
          <div style={{ position: "relative", marginBottom: 20 }}>
            ...
            <input type="text" placeholder="Rechercher parmi les enseignants..." value={search} onChange={e => setSearch(e.target.value)}
              style={{ width: "100%", padding: "12px 14px 12px 42px", border: "1px solid #e2e8f0", borderRadius: 12, fontSize: 13, outline: "none", boxSizing: "border-box", background: "#fff" }} />
          </div>
            ...
            {employes.filter(e => e.categorie === "Enseignant" && e.nom.toLowerCase().includes(search.toLowerCase())).map(t => (
              <motion.div key={t.id} whileHover={{ y: -4, boxShadow: "0 12px 20px rgba(0,0,0,0.05)" }} style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "20px", display: "flex", flexDirection: "column", gap: 14, position: "relative" }}>
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: "rgba(59, 130, 246, 0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>
                    {t.avatar}
                  </div>
                  <div>
                    <h3 style={{ fontSize: 15, fontWeight: 800, margin: 0, color: "#1e293b" }}>{t.nom}</h3>
                    <span style={{ fontSize: 11, color: "#3b82f6", fontWeight: 700 }}>{t.poste}</span>
                  </div>
                </div>
                ...
                <div style={{ background: "#f8fafc", borderRadius: 12, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 12, color: "#64748b", fontWeight: 600 }}>Évaluation Pédagogique :</span>
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <span style={{ color: "#f59e0b", fontSize: 14 }}>★</span>
                    <span style={{ fontSize: 13, fontWeight: 800, color: "#0f172a" }}>{t.evaluation} / 5</span>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button onClick={() => { setSelectedStaff(t); setDossierTab("planning"); }} style={{ flex: 1, padding: "8px 0", background: "#f1f5f9", color: "#475569", border: "none", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer", transition: "0.2s" }}>
                    📅 Emploi du Temps
                  </button>
                  <button onClick={() => { setSelectedStaff(t); setDossierTab("profil"); }} style={{ flex: 1, padding: "8px 0", background: "#3b82f6", color: "#fff", border: "none", borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: "pointer", transition: "0.2s" }}>
                    👁️ Dossier complet
                  </button>
                </div>
              </motion.div>
      ...
      {activeTab === "presences" && (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "24px" }}>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: "#1e3a8a", margin: "0 0 16px 0", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
              <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, padding: 12, textAlign: "center" }}>
                <span style={{ fontSize: 11, color: "#166534", fontWeight: 700, display: "block" }}>PRÉSENTS TODAY</span>
                <span style={{ fontSize: 20, fontWeight: 800, color: "#15803d", marginTop: 4, display: "block" }}>
              ...
              <div style={{ background: "#fffbeb", border: "1px solid #fef08a", borderRadius: 10, padding: 12, textAlign: "center" }}>
                <span style={{ fontSize: 11, color: "#854d0e", fontWeight: 700, display: "block" }}>RETARDS TODAY</span>
                <span style={{ fontSize: 20, fontWeight: 800, color: "#a16207", marginTop: 4, display: "block" }}>
              ...
              <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: 12, textAlign: "center" }}>
                <span style={{ fontSize: 11, color: "#991b1b", fontWeight: 700, display: "block" }}>ABSENTS TODAY</span>
                <span style={{ fontSize: 20, fontWeight: 800, color: "#b91c1c", marginTop: 4, display: "block" }}>
                ...
                return (
                  <div key={e.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: 12, borderRadius: 12, border: "1px solid #f1f5f9", background: "#f8fafc" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        <strong style={{ fontSize: 13, color: "#1e293b", display: "block" }}>{e.nom}</strong>
                        <span style={{ fontSize: 11, color: "#64748b" }}>{e.poste}</span>
                      </div>
                    </div>
          ...
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "20px" }}>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: "#1e3a8a", margin: "0 0 14px 0" }}>📜 Historique des Présences</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {attendanceLogs.map((log, idx) => (
                <div key={idx} style={{ padding: 12, borderRadius: 10, border: "1px solid #f1f5f9", background: "#fafafa" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#334155" }}>{log.date}</span>
      ...
      {activeTab === "conges" && (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20 }}>
          ...
          {/* DEMANDES DE CONGES */}
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "24px" }}>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: "#ea580c", margin: "0 0 16px 0" }}>🌴 Demandes &amp; Absences Planifiées</h2>
                  ...
                  {leaveRequests.map(r => (
                    <tr key={r.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                      <td style={{ padding: "12px 14px" }}>
                        <strong>{r.nom}</strong>
                        <span style={{ fontSize: 10, color: "#64748b", display: "block" }}>{r.poste}</span>
                      </td>
                      <td style={{ padding: "12px 14px" }}>
                        <span style={{ padding: "2px 6px", borderRadius: 4, background: "#f1f5f9", color: "#475569", fontSize: 11, fontWeight: 600 }}>{r.type}</span>
                      </td>
                      ...
                      <td style={{ padding: "12px 14px" }}>
                        {r.statut === "En attente" ? (
                          <div style={{ display: "flex", gap: 6 }}>
                            <button onClick={() => handleLeaveDecision(r.id, true)} style={{ background: "#dcfce7", color: "#15803d", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>Approuver</button>
                            <button onClick={() => handleLeaveDecision(r.id, false)} style={{ background: "#fee2e2", color: "#b91c1c", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>Refuser</button>
                          </div>
                        ) : (
                          <span style={{ fontSize: 11, color: "#94a3b8" }}>Traité</span>
                        )}
                      </td>
                    </tr>
                  ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "20px" }}>
              <h3 style={{ fontSize: 14, fontWeight: 800, color: "#ea580c", margin: "0 0 14px 0" }}>🌴 Soldes de Congés Restants</h3>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 20 }}>
            <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 12, padding: 16 }}>
              <span style={{ fontSize: 11, color: "#1e40af", fontWeight: 700, display: "block" }}>MASSE DE BASE</span>
              <strong style={{ fontSize: 18, color: "#1d4ed8", display: "block", marginTop: 4 }}>{totalSalaires.toLocaleString()} GNF</strong>
            </div>
            <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: 16 }}>
              <span style={{ fontSize: 11, color: "#166534", fontWeight: 700, display: "block" }}>PRIMES DU MOIS</span>
              <strong style={{ fontSize: 18, color: "#15803d", display: "block", marginTop: 4 }}>{employes.reduce((sum, e) => sum + e.primes, 0).toLocaleString()} GNF</strong>
            </div>
            <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: 16 }}>
              <span style={{ fontSize: 11, color: "#991b1b", fontWeight: 700, display: "block" }}>RETENUES CONSTATÉES</span>
              <strong style={{ fontSize: 18, color: "#b91c1c", display: "block", marginTop: 4 }}>{employes.reduce((sum, e) => sum + e.retenues, 0).toLocaleString()} GNF</strong>
            </div>
            <div style={{ background: "#fdf2f8", border: "1px solid #fbcfe8", borderRadius: 12, padding: 16 }}>
              <span style={{ fontSize: 11, color: "#9d174d", fontWeight: 700, display: "block" }}>NET TOTAL À PAYER</span>
              <strong style={{ fontSize: 18, color: "#be185d", display: "block", marginTop: 4 }}>{employes.reduce((sum, e) => sum + (e.salaire + e.primes - e.retenues), 0).toLocaleString()} GNF</strong>
            </div>
          </div>
                  ...
                  return (
                    <tr key={e.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                      <td style={{ padding: "14px 18px" }}>
                        ...
                        <span style={{ fontSize: 10, color: "#64748b" }}>{e.poste}</span>
                      </td>
                      ...
                      <td style={{ padding: "14px 18px" }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          {e.statutPaie === "En attente" && (
                            <button onClick={() => handlePaySalary(e.id)} style={{ background: "#22c55e", color: "#fff", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                              💵 Payer
                            </button>
                          )}
                          <button onClick={() => setSelectedPayslip(e)} style={{ background: "#f1f5f9", color: "#475569", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
                            📄 Bulletin
                          </button>
                        </div>
                      </td>
      ...
      {activeTab === "recrutement" && (
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
            {[
              { id: "CV", label: "📥 CVs Reçus", color: "#3b82f6", bg: "rgba(59, 130, 246, 0.05)" },
              { id: "Entretien", label: "💬 Entretiens en cours", color: "#f59e0b", bg: "rgba(245, 158, 11, 0.05)" },
              { id: "Offres", label: "🤝 Offre formulée", color: "#8b5cf6", bg: "rgba(139, 92, 246, 0.05)" },
              { id: "Engage", label: "🎉 Recruté(e)s", color: "#10b981", bg: "rgba(16, 185, 129, 0.05)" },
            ].map(col => {
              ...
              return (
                <div key={col.id} style={{ background: col.bg, border: `1px solid ${col.color}20`, borderRadius: 16, padding: "16px", minHeight: 480 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    ...
                    <span style={{ background: col.color, color: "#fff", borderRadius: 20, padding: "2px 8px", fontSize: 11, fontWeight: 700 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {colCandidates.map(cand => (
                      <motion.div key={cand.id} whileHover={{ y: -3, boxShadow: "0 8px 16px rgba(0,0,0,0.04)" }} style={{ background: "#fff", borderRadius: 12, border: "1px solid #cbd5e1", padding: "14px", display: "flex", flexDirection: "column", gap: 8 }}>
                        <div>
                          <h4 style={{ fontSize: 13, fontWeight: 800, color: "#1e293b", margin: 0 }}>{cand.nom}</h4>
                          <span style={{ fontSize: 11, color: "#64748b", display: "block", marginTop: 2 }}>{cand.poste}</span>
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
                            <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "#f1f5f9", color: "#475569", fontWeight: 700 }}>
                              💼 {cand.categorie || "Enseignant"}
                            </span>
                            <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: col.bg, color: col.color, fontWeight: 700 }}>
                              🎓 {cand.diplome}
                            </span>
                          </div>
                        </div>
                        
                        <div style={{ fontSize: 10, color: "#94a3b8", display: "flex", flexDirection: "column", gap: 2, borderTop: "1px solid #f1f5f9", paddingTop: 8 }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "flex-end", gap: 6, marginTop: 8 }}>
                          {col.id === "CV" && (
                            <button onClick={() => handleMoveCandidate(cand.id, "Entretien")} style={{ background: "#fef9c3", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, color: "#854d0e", cursor: "pointer" }}>
                              💬 Entretien ➔
                            </button>
                          )}
                          {col.id === "Entretien" && (
                            <div style={{ display: "flex", gap: 4 }}>
                              <button onClick={() => handleMoveCandidate(cand.id, "CV")} style={{ background: "#f1f5f9", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, color: "#64748b", cursor: "pointer" }}>
                                🞀 Retour
                              </button>
                              <button onClick={() => handleMoveCandidate(cand.id, "Offres")} style={{ background: "#f3e8ff", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, color: "#6b21a8", cursor: "pointer" }}>
                                🤝 Offre ➔
                              </button>
                            </div>
                          )}
                          {col.id === "Offres" && (
                            <div style={{ display: "flex", gap: 4 }}>
                              <button onClick={() => handleMoveCandidate(cand.id, "Entretien")} style={{ background: "#f1f5f9", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, color: "#64748b", cursor: "pointer" }}>
                                🞀 Retour
                              </button>
                              <button onClick={() => handleMoveCandidate(cand.id, "Engage")} style={{ background: "#dcfce7", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, color: "#166534", cursor: "pointer" }}>
                                🎉 Recruter !
                              </button>
                            </div>
                          )}
                          {col.id === "Engage" && (
                            <span style={{ fontSize: 10, color: "#10b981", fontWeight: 700 }}>✅ Prêt à l'embauche</span>
                          )}
                        </div>
                      </motion.div>
                    ))}
                    {colCandidates.length === 0 && (
                      <div style={{ border: "2px dashed #cbd5e1", borderRadius: 12, padding: "20px 10px", textAlign: "center", color: "#94a3b8", fontSize: 11 }}>
        ...
        {selectedStaff && (
          <div className="modal-overlay" onClick={() => setSelectedStaff(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="modal-content" onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: 560, overflow: "hidden", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)" }}>
              ...
              {/* MODAL HEADER */}
              <div style={{ background: "linear-gradient(135deg, #1e3a8a, #3b82f6)", padding: "24px", color: "#fff", position: "relative" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                    <div style={{ width: 58, height: 58, borderRadius: 16, background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>
                      {selectedStaff.avatar}
                    </div>
                    <div>
                      <h2 style={{ fontSize: 18, fontWeight: 800, margin: 0 }}>{selectedStaff.nom}</h2>
                      <span style={{ fontSize: 12, color: "#bfdbfe", fontWeight: 600 }}>{selectedStaff.poste}</span>
                    </div>
                  </div>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 20, borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: 14 }}>
                  {[
                    { id: "profil", label: "👤 Profil & Contrat" },
                    ...
                    <button key={tab.id} onClick={() => setDossierTab(tab.id)} style={{
                      ...
                      border: "none", padding: "6px 12px", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", transition: "all 0.2s"
                    }}>
                      {tab.label}
                    </button>
                  ))}
              ...
              <div className="modal-body" style={{ padding: "24px", maxHeight: "60vh", overflowY: "auto" }}>
                ...
                {dossierTab === "profil" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      <div>
                        <span style={{ fontSize: 11, color: "#94a3b8", display: "block" }}>Téléphone</span>
                        <strong style={{ fontSize: 13, color: "#334155" }}>{selectedStaff.telephone}</strong>
                      </div>
                      <div>
                        <span style={{ fontSize: 11, color: "#94a3b8", display: "block" }}>E-mail</span>
                        <strong style={{ fontSize: 13, color: "#334155" }}>{selectedStaff.email}</strong>
                      </div>
                      <div>
                        <span style={{ fontSize: 11, color: "#94a3b8", display: "block" }}>Dernier Diplôme</span>
                        <strong style={{ fontSize: 13, color: "#334155" }}>{selectedStaff.diplome}</strong>
                      </div>
                      <div>
                        <span style={{ fontSize: 11, color: "#94a3b8", display: "block" }}>Contrat &amp; Ancienneté</span>
                        <strong style={{ fontSize: 13, color: "#334155" }}>{selectedStaff.contrat} ({formatSeniority(selectedStaff.dateEmbauche)})</strong>
                      </div>
                    </div>

                    <div style={{ background: "#f8fafc", borderRadius: 12, padding: 14, border: "1px solid #f1f5f9" }}>
                      <h4 style={{ margin: "0 0 8px 0", fontSize: 12, fontWeight: 800, color: "#475569" }}>💵 Informations Financières</h4>
                    ...
                    <div>
                      <h4 style={{ margin: "0 0 8px 0", fontSize: 12, fontWeight: 800, color: "#475569" }}>📁 Documents administratifs joints</h4>
                        ...
                        {[
                          ...
                          <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", border: "1px solid #e2e8f0", borderRadius: 8, background: "#fafafa" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              ...
                              <div>
                                <span style={{ fontSize: 12, color: "#334155", fontWeight: 600, display: "block" }}>{doc.nom}</span>
                                <span style={{ fontSize: 10, color: "#94a3b8" }}>{doc.taille}</span>
                              </div>
                            </div>
                            <button onClick={() => alert(`Téléchargement simulé de ${doc.nom}`)} style={{ background: "none", border: "none", cursor: "pointer", color: "#3b82f6", fontSize: 12, fontWeight: 700 }}>Télécharger</button>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {dossierTab === "planning" && (
                  <div>
                    <h4 style={{ margin: "0 0 12px 0", fontSize: 13, fontWeight: 800, color: "#1e3a8a" }}>📅 Emploi du Temps Hebdomadaire</h4>
                      ...
                      {daysOfWeek.map(day => {
                        ...
                        return (
                          <div key={day} style={{ 
                            ...
                            <span style={{ fontSize: 11, fontWeight: 800, color: hours ? "#1e40af" : "#94a3b8" }}>{day}</span>
                            {hours ? (
                              <div style={{ fontSize: 10, fontWeight: 700, color: "#2563eb", background: "#fff", padding: "4px 6px", borderRadius: 4, marginTop: 8, textAlign: "center" }}>
                                {hours}
                              </div>
                            ) : (
                              <span style={{ fontSize: 10, color: "#cbd5e1", fontStyle: "italic", marginTop: 8, display: "block", textAlign: "center" }}>Libre</span>
                            )}
                          </div>
                        );
                      })}
                ...
                {dossierTab === "historique" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    <h4 style={{ margin: "0 0 4px 0", fontSize: 13, fontWeight: 800, color: "#1e3a8a" }}>📜 Historique de Carrière &amp; Parcours</h4>
                    ...
                    <div style={{ position: "relative", borderLeft: "2px solid #e2e8f0", marginLeft: 10, paddingLeft: 16, display: "flex", flexDirection: "column", gap: 16 }}>
                      {[
                        ...
                        <div key={idx} style={{ position: "relative" }}>
                          ...
                          <span style={{ fontSize: 10, color: "#94a3b8", fontWeight: 700 }}>{item.date}</span>
                          <strong style={{ fontSize: 12, color: "#334155", display: "block", marginTop: 2 }}>{item.titre}</strong>
          ...
          <div className="modal-overlay" onClick={() => setSelectedPayslip(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 350 }}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="modal-content" onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 16, width: 440, padding: 24, boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)", border: "1px dashed #cbd5e1" }}>
              ...
              <div style={{ textAlign: "center", borderBottom: "1px dashed #cbd5e1", paddingBottom: 14, marginBottom: 14 }}>
                <strong style={{ fontSize: 16, color: "#1e3a8a", display: "block" }}>🏢 SCHOOLX GROUP ACADEMY</strong>
                <span style={{ fontSize: 11, color: "#64748b" }}>République de Guinée — Conakry</span>
                <h3 style={{ margin: "10px 0 0 0", fontSize: 14, fontWeight: 800, color: "#0f172a" }}>BULLETIN DE PAIE — MAI 2026</h3>
              </div>

              {/* EMPLOYEE INFO */}
              <div style={{ fontSize: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14, background: "#f8fafc", padding: 10, borderRadius: 8 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12, borderBottom: "1px dashed #cbd5e1", paddingBottom: 12, marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 800, color: "#1e3a8a", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setSelectedPayslip(null)} style={{ flex: 1, padding: 10, border: "1px solid #cbd5e1", borderRadius: 8, background: "#fff", color: "#64748b", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Fermer</button>
                <button onClick={() => { alert("Impression simulée déclenchée !"); setSelectedPayslip(null); }} style={{ flex: 1, padding: 10, border: "none", borderRadius: 8, background: "#1e3a8a", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>🖨️ Imprimer Bulletin</button>
              </div>
            </motion.div>
          </div>
        )}
          ...
          <div className="modal-overlay" onClick={() => setShowAddLeaveModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="modal-content" onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: 420, overflow: "hidden", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)" }}>
              <div style={{ background: "#ea580c", padding: "20px 24px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 style={{ fontSize: 17, fontWeight: 800, margin: 0 }}>🌴 Enregistrer une demande de congé</h2>
                <button className="modal-close" onClick={() => setShowAddLeaveModal(false)} style={{ background: "none", border: "none", color: "#fff", fontSize: 20, cursor: "pointer" }}>✕</button>
              </div>

              <div className="modal-body" style={{ padding: "24px" }}>
                <div style={{ display: "grid", gap: 14 }}>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Employé concerné *</label>
                    <select value={newLeaveForm.employeId} onChange={e => setNewLeaveForm({ ...newLeaveForm, employeId: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }}>
                  ...
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Type de congé</label>
                    <select value={newLeaveForm.type} onChange={e => setNewLeaveForm({ ...newLeaveForm, type: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Date Début</label>
                      <input type="date" value={newLeaveForm.debut} onChange={e => setNewLeaveForm({ ...newLeaveForm, debut: e.target.value })} style={{ width: "100%", padding: "9px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                    </div>
                    <div>
                      <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Date Fin</label>
                      <input type="date" value={newLeaveForm.fin} onChange={e => setNewLeaveForm({ ...newLeaveForm, fin: e.target.value })} style={{ width: "100%", padding: "9px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                    </div>
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Nombre de jours ouvrés *</label>
                    <input type="number" placeholder="Ex: 5" value={newLeaveForm.jours} onChange={e => setNewLeaveForm({ ...newLeaveForm, jours: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                </div>

                <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
                  <button onClick={() => setShowAddLeaveModal(false)} style={{ flex: 1, padding: 12, border: "1px solid #cbd5e1", borderRadius: 10, background: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#64748b" }}>Annuler</button>
                  <button onClick={handleAddLeave} style={{ flex: 1, padding: 12, border: "none", borderRadius: 10, background: "#ea580c", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Soumettre</button>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="modal-overlay" onClick={() => setShowAddStaffModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="modal-content" onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: 460, overflow: "hidden", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)" }}>
              ...
              <div style={{ background: "#1e3a8a", padding: "20px 24px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 style={{ fontSize: 18, fontWeight: 800, margin: 0 }}>👤 Ajouter un Nouveau Collaborateur</h2>
                <button className="modal-close" onClick={() => setShowAddStaffModal(false)} style={{ background: "none", border: "none", color: "#fff", fontSize: 20, cursor: "pointer" }}>✕</button>
              </div>
              ...
              <div className="modal-body" style={{ padding: "24px", maxHeight: "75vh", overflowY: "auto" }}>
                <div style={{ display: "grid", gap: 14 }}>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Nom complet *</label>
                    <input type="text" placeholder="Ex: Jean Martin" value={newStaffForm.nom} onChange={e => setNewStaffForm({ ...newStaffForm, nom: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Poste / Discipline d'enseignement *</label>
                    <input type="text" placeholder="Ex: Professeur de Mathématiques ou Cuisinier" value={newStaffForm.poste} onChange={e => setNewStaffForm({ ...newStaffForm, poste: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Catégorie de rôle *</label>
                    <select value={newStaffForm.categorie} onChange={e => setNewStaffForm({ ...newStaffForm, categorie: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Type de contrat</label>
                      <select value={newStaffForm.contrat} onChange={e => setNewStaffForm({ ...newStaffForm, contrat: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }}>
                    ...
                    <div>
                      <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Salaire mensuel (GNF) *</label>
                      <input type="number" placeholder="Ex: 2000000" value={newStaffForm.salaire} onChange={e => setNewStaffForm({ ...newStaffForm, salaire: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                    </div>
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Dernier Diplôme obtenu</label>
                    <input type="text" placeholder="Ex: Master en Administration" value={newStaffForm.diplome} onChange={e => setNewStaffForm({ ...newStaffForm, diplome: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Téléphone</label>
                      <input type="text" placeholder="Ex: +224 620..." value={newStaffForm.telephone} onChange={e => setNewStaffForm({ ...newStaffForm, telephone: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                    </div>
                    <div>
                      <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>E-mail</label>
                      <input type="email" placeholder="Ex: j.martin@schoolx.gn" value={newStaffForm.email} onChange={e => setNewStaffForm({ ...newStaffForm, email: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                    </div>
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Avatar (Emoji)</label>
                    <select value={newStaffForm.avatar} onChange={e => setNewStaffForm({ ...newStaffForm, avatar: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }}>
                  ...
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Emploi du temps (Séparez par des points-virgules ';')</label>
                    <input type="text" placeholder="Ex: Lundi (08h - 12h); Mercredi (10h - 14h)" value={newStaffForm.emploiDuTemps} onChange={e => setNewStaffForm({ ...newStaffForm, emploiDuTemps: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                </div>

                <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
                  <button onClick={() => setShowAddStaffModal(false)} style={{ flex: 1, padding: 12, border: "1px solid #cbd5e1", borderRadius: 10, background: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#64748b" }}>Annuler</button>
                  <button onClick={handleAddEmployee} style={{ flex: 1, padding: 12, border: "none", borderRadius: 10, background: "#1e3a8a", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Créer l'employé</button>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="modal-overlay" onClick={() => setShowAddCandidateModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300 }}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="modal-content" onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: 440, overflow: "hidden", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)" }}>
              ...
              <div style={{ background: "#8b5cf6", padding: "20px 24px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 style={{ fontSize: 18, fontWeight: 800, margin: 0 }}>📥 Nouveau Dossier Candidat</h2>
                <button className="modal-close" onClick={() => setShowAddCandidateModal(false)} style={{ background: "none", border: "none", color: "#fff", fontSize: 20, cursor: "pointer" }}>✕</button>
              </div>
              ...
              <div className="modal-body" style={{ padding: "24px", maxHeight: "65vh", overflowY: "auto" }}>
                <div style={{ display: "grid", gap: 14 }}>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Nom complet du Candidat *</label>
                    <input type="text" placeholder="Ex: Marc Dubois" value={newCandidateForm.nom} onChange={e => setNewCandidateForm({ ...newCandidateForm, nom: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Poste ciblé *</label>
                    <input type="text" placeholder="Ex: Professeur de Chimie ou Cuisinier" value={newCandidateForm.poste} onChange={e => setNewCandidateForm({ ...newCandidateForm, poste: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Catégorie de rôle *</label>
                    <select value={newCandidateForm.categorie} onChange={e => setNewCandidateForm({ ...newCandidateForm, categorie: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }}>
                      <option value="Enseignant">Enseignant</option>
                      <option value="Administration">Administration (Direction, Comptable, Secrétaire, etc.)</option>
                  ...
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Dernier Diplôme &amp; Titres</label>
                    <input type="text" placeholder="Ex: Doctorat en Chimie Appliquée" value={newCandidateForm.diplome} onChange={e => setNewCandidateForm({ ...newCandidateForm, diplome: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Adresse E-mail</label>
                    <input type="email" placeholder="Ex: m.dubois@gmail.com" value={newCandidateForm.email} onChange={e => setNewCandidateForm({ ...newCandidateForm, email: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                  <div>
                    <label style={{ display: "block", marginBottom: 4, fontWeight: 600, fontSize: 12, color: "#64748b" }}>Téléphone Direct</label>
                    <input type="text" placeholder="Ex: +224 624..." value={newCandidateForm.tel} onChange={e => setNewCandidateForm({ ...newCandidateForm, tel: e.target.value })} style={{ width: "100%", padding: "10px", border: "1px solid #e2e8f0", borderRadius: 8, outline: "none", fontSize: 13 }} />
                  </div>
                </div>

                <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
                  <button onClick={() => setShowAddCandidateModal(false)} style={{ flex: 1, padding: 12, border: "1px solid #cbd5e1", borderRadius: 10, background: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#64748b" }}>Annuler</button>
                  <button onClick={handleAddCandidate} style={{ flex: 1, padding: 12, border: "none", borderRadius: 10, background: "#8b5cf6", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Ajouter au Pipeline</button>
                </div>
              </div>
            </motion.div>
          </div>
